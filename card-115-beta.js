...
class SajH2InverterCard extends HTMLElement {
  ...
  constructor() {
    ...
    console.log(`[SAJ H2 Inverter Card] Version: 1.1.5-beta`);
  ...
  set hass(hass) {
    ...
    if (this.shadowRoot && (shouldUpdate || !this.shadowRoot.innerHTML) && !userInteracting) {
        // Introduce a small delay before re-rendering if a time input was just interacted with
        if (this._lastInteractedTimeInput) {
            setTimeout(() => {
                this._renderCard();
                this._lastInteractedTimeInput = false; // Reset flag after rendering
            }, 100); // 100ms delay
        } else {
            this._renderCard();
        }
  ...
  _isUserInteracting() {
    ...
    // Protect time inputs from re-renders while focused
    if (activeElement.tagName === 'INPUT' && activeElement.type.toLowerCase() === 'time') {
      this._lastInteractedTimeInput = true; // Set flag to indicate time input was interacted with
      return true; // Currently interacting with a time input
    }
    // Protect range inputs from re-renders while focused
    if (activeElement.tagName === 'INPUT' && activeElement.type.toLowerCase() === 'range') {
      return true; // Currently interacting with a range input
    }
